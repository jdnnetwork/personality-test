...
// validate_company에 한해 few-shot multi-turn 예시 삽입 (모델이 실제 대화 패턴을 학습)
const VALIDATE_FEW_SHOTS = [
  { role: "user",      content: '입력: "삼성전자"' },
  { role: "assistant", content: '{"valid":true,"correctedName":"삼성전자","message":null}' },
  { role: "user",      content: '입력: "네이버"' },
  { role: "assistant", content: '{"valid":true,"correctedName":"네이버","message":null}' },
  { role: "user",      content: '입력: "카카오"' },
  { role: "assistant", content: '{"valid":true,"correctedName":"카카오","message":null}' },
  { role: "user",      content: '입력: "현대자동차"' },
  { role: "assistant", content: '{"valid":true,"correctedName":"현대자동차","message":null}' },
  { role: "user",      content: '입력: "한국전력공사"' },
  { role: "assistant", content: '{"valid":true,"correctedName":"한국전력공사","message":null}' },
  { role: "user",      content: '입력: "현대차"' },
  { role: "assistant", content: '{"valid":true,"correctedName":"현대자동차","message":"현대자동차로 검색합니다"}' },
  { role: "user",      content: '입력: "SKT"' },
  { role: "assistant", content: '{"valid":true,"correctedName":"SK텔레콤","message":"SK텔레콤으로 검색합니다"}' },
  { role: "user",      content: '입력: "삼성"' },
  { role: "assistant", content: '{"valid":true,"correctedName":"삼성전자","message":"삼성전자로 검색합니다"}' },
  { role: "user",      content: '입력: "asdfqwer"' },
  { role: "assistant", content: '{"valid":false,"correctedName":null,"message":"해당 기업을 찾을 수 없습니다. 정확한 기업명을 입력해주세요."}' },
];
...
- 응답 패턴 참고 사항이 있으면 위 20개 중 1~2개를 해당 패턴을 자연스럽게 검증하는 질문으로 대체 가능 (단 '비빈도/IF' 같은 전문 용어는 질문 텍스트에 쓰지 말 것)`;
      ...
      systemPrompt = `당신은 취업 지원 맥락에서 한국 기업명을 검증하는 도우미입니다. 반드시 유효한 JSON만 출력하세요. 설명·주석·코드블록 없이 JSON 객체 하나만 응답합니다.

**매우 관대하게** 판정합니다. 취업 지원자가 입력하는 이름은 99% 실존 기업입니다. 한국에서 일반 상식으로 알려진 기업이면 모두 valid: true로 처리하세요. valid: false는 입력이 명백한 무의미 문자열("asdfqwer", "ㅁㄴㅇㄹ", "aaa")일 때만 사용합니다. 모르겠으면 valid: true로 기울여 판정하세요.
