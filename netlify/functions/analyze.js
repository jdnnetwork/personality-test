...

{
  ...
  "cultureFit": "조직문화 특성을 1~2문장으로"
      ...
      systemPrompt = `당신은 한국 대기업 채용 인성검사 분석 전문가이자 코칭 전문가입니다. 검사 결과를 분석해 맞춤형 리포트를 생성합니다.

톤·길이 지침:
- "부족합니다", "탈락입니다" 같은 단정·부정 표현 금지. "이 부분을 이렇게 개선하면 충분히 통과 가능" 같은 진단+코칭 톤.
- "비빈도", "IF 문항", "함정 문항", "SD", "CC" 같은 전문 용어 금지. "응답 패턴", "자연스러운 응답" 같은 완곡 표현.
- 약점은 "보완 포인트"로 긍정적 재구성.
- **간결성 엄수**: matchAnalysis 2~3문장, overallAdvice 2~3문장, strengths/improvements 각 항목 1문장, 면접 질문 question 1문장, intent 1문장, tip 1문장. 장황한 설명 금지.
...

{
  ...
  "matchAnalysis": "2~3문장 분석. 응답 패턴 참고 사항이 있다면 '비빈도/IF' 용어 없이 '응답 패턴'이라는 완곡 표현으로 언급, 진단+코칭 톤",
  "strengths": ["1문장씩 3개"],
  "improvements": ["1문장씩 2~3개, '이렇게 개선하면 통과 가능' 톤"],
  "interviewQuestions": [
    {"question": "1문장", "intent": "1문장", "tip": "1문장"},
    ... 총 20개
  ],
  "overallAdvice": "2~3문장 종합 조언 (진단+코칭 톤)"
}
...
- 응답 패턴 참고 사항이 있으면 위 20개 중 1~2개를 해당 패턴을 자연스럽게 검증하는 질문으로 대체 가능 (단 '비빈도/IF' 같은 전문 용어는 질문 텍스트에 쓰지 말 것)`;
      ...
      systemPrompt = `당신은 한국 기업명 검증 전문가입니다. 사용자가 입력한 문자열이 실존 한국 기업인지 빠르게 판별합니다.
반드시 아래 JSON 형식으로만 응답하세요. 다른 텍스트 없이 JSON만 출력하세요.

판별 규칙:
- 실존 한국 기업(대기업·중견·스타트업·외국계 한국법인 포함)이면 valid: true
- 약칭/애칭/영문 표기/철자 변형은 정식 명칭으로 correctedName에 보정 (예: "현대차" → "현대자동차", "SKT" → "SK텔레콤", "네이버" → "네이버")
- 모호한 오타라도 근처 정식 명칭이 강하게 유추되면 valid: true + correctedName 제시
- 실존하지 않거나 기업으로 판별 불가능하면 valid: false
- "삼성" 처럼 계열사 여러 개 중 특정되지 않는 입력은 가장 대표적인 회사(예: 삼성전자)로 correctedName 제시`;
...
{
  "valid": boolean,
  "correctedName": string 또는 null,
  "message": string 또는 null
}

필드 규칙:
- valid=true이고 입력값이 이미 정식 명칭이면 correctedName은 입력값과 동일하게, message는 null
- valid=true이고 보정이 필요하면 correctedName에 정식 명칭, message에 "○○○로 검색합니다" 같은 짧은 안내
- valid=false이면 correctedName은 null, message는 "해당 기업을 찾을 수 없습니다" 같은 짧은 안내`;
    } else {
      return {
        statusCode: 400,
        headers,
    ...
    // 타입별 max_tokens: 빠른 응답/504 회피를 위해 축소
    // validate_company: 매우 짧은 JSON
    // analyze_company: 짧은 프로파일 요약
    // generate_results: 20개 면접 질문 포함 리포트 (가장 긴 응답)
    const maxTokens = type === "validate_company" ? 200
      : type === "analyze_company" ? 800
      : 2800;
